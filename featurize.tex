...
\begin{equation}
  ...
  \label{eq:pck}
\end{equation}
We set the Zipf proportion to $q = 0.4$ in the subsequent examples.  The Zipf component $p^z$ is common to attributes and hence not useful to distinguish the attributes in a document. The multinomial component in \eqn{pck} is randomly sampled from a Dirichlet distribution of dimension $m_c$, independently for each topic.  The second component of the distribution over words for an attribute is also randomly generated by again sampling from a very spiked Dirichlet distribution over the remaining $m-m_c$ words. 
 \begin{equation*}
