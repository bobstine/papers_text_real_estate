...
\end{table}
 ...
 principal components regression.  Section \ref{sec:tokenspace} presents vector space
 models such as LSA from from the novel perspective that we call ``token space.'' Token space characterizes text as points in a very high dimensional space that restores the statistical sense 
 of ``observations,'' samples drawn from a population.  In token space, LSA is seen to
...
Token space is the vector space generated by modeling the sequence of word tokens as indicator vectors.  From this perspective, the frequencies in the document-word matrix $W$ become covariances between numerical random variables produced by the observed sequence of tokens. Token space should should feel very familiar to statisticians and provides motivation and heuristics for creating a wide range of features from text.  


Recall that we observe $n$ real-estate listings that we will now generically call `documents'.   Document $i$ is a sequence of $n_i$ word tokens that have been chosen from a vocabulary of $m$ word types.  To distinguish word types from word tokens, we use the symbol $\omega$ to denote types and $w$ to denote tokens.  We model each document as an independent observation of a stochastic process $H(\theta)$ that produces a sequence of a random number of word tokens.  The $i$th document is thus a sequence $\{w_1, w_2, \ldots, w_{n_i}\}$ with words chosen from the available vocabulary, $w_i \in V$.   Each document is allowed to have its own set of parameter values that control the stochastic process.  Given  parameters $\theta_i$, each observation of $H(\theta_i)$ is a sequence of word tokens.  Section \ref{sec:topic} gives an example of such a process.    The word tokens of a document may be independent in some applications, but dependence is more likely\citep[\eg][]{fosterkakade07}.
...
\end{equation*}
with total length $N = \sum n_i$.  An analogous common statistical model  describes longitudinal data in which, for example, we observe a varying number of measurements per subject.  The subjects are usually modeled as independent, but the repeated measurements are not.  These measurements are often numerical, such as a sequence of blood pressures.  It is not uncommon as well to find that the number of measurements is associated with the health of subjects; sicker patients might be more closely observed than those with less severe illness.


Token space simply represents the list $\ell$ as a very sparse matrix formed by indicators.   From $\ell$, define the $N \times m$  matrix $X$ in which $X_{ij} = 1$ if the $i$th word token in $\ell$ is of type $j$:
\begin{equation}
...
\begin{equation}
  ...
  \end{array}
    ...
    \label{eq:X}
\end{equation}
This matrix is equivalent to the original text.  


This conversion produces more than numbers.  It also changes the way we interpret common summaries such as word frequencies. Though a trivial conversion, this representation of text frames the modeling as the analysis of a sparse numerical matrix rather than a sequence of words.  The document-type matrix $W$ used in LSA, for example, is seen to be a covariance matrix.  Let the $N \times n$ matrix $L$  identify the documents,
\begin{equation}
  L =  \left(  
           \begin{array}{ccccc}
            1  & 0 & 0 & \cdots & 0 \cr
