...
The presence of a response distinguishes our model from topic models.  In machine learning, topic modeling is an unsupervised algorithm typically used to cluster documents based on the presence of latent topics revealed by a hierarchical Bayesian  model.  Our method of featurizing text is also unsupervised,  but our goal is to predict the explicit response rather than identify latent clusters. Topic models do not describe a response.  For example, standard topic models do not describe the connection between document length and the response.  In a topic model, the length of a document does not reveal the number of topics  present in the document.  This characteristics is pronounced, however,  in real estate listings.  Authors of listings write more about valuable properties than cheaper properties, and are motivated to write more about expensive properties, those with numerous attractive attributes worth mentioning.  A listing summarizes desirable attributes that are present, such as a fireplace, not those that are missing.   An ad would not mention that a property did not have a fireplace.  The  model developed in the following section captures this length effect as well as the other stylized facts observed in the listings.  We describe the model in the context of real-estate listings, and generalization to other applications is straightforward.
...
The collection of attributes also determines the text of the associated listing, providing the link that allows one to predict the price given the text of a listing.  As in a standard topic model, each attribute defines a multinomial distribution over the vocabulary of words.  The words in a listing are then sampled according to the mixture of distributions determined by its attributes.  

HERE

We model the distribution over words for each attribute with two components.  One component is related to the response, and the other captures common words shared by all attributes.  This common distribution is a Zipf distribution over the vocabulary.  A second, distribution distinguishes the attributes.  Divide the vocabulary of $m$ words into $m_c$ common words shared by topics and $m - m_c$ words that are randomly sampled for each topic.  The distribution for an attribute convolves a Zipf distribution over the vocabulary with a topic-specific multinomial distribution over the $m - m_c$ attribute-specific words. Let 
\begin{equation*}
