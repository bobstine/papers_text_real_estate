...
% ----------------------------------------------------------------------
 ...
 As a specific example, we build a regression model to predict the advertised
 prices of real estate properties from their listings.  The listings we use are
 verbal rather than numerical data such as that obtained by filling out a
 spreadsheet-like questionnaire.  The listings may, or may not, contain
 quantitative characteristics.  The raw data are $n=$7,384 property listings for
 Chicago, IL extracted (with permission) from trulia.com on June 12, 2013.  (At
 the time, trulia.com showed 30,322 listings for Chicago, but most were
 foreclosures that we excluded from our analysis.) The response in our models is
 the log of the listed price.  The distribution of the listed prices is right
 skewed, so we transformed the response to a log scale as shown in the histogram
 of Figure \ref{fig:prices}.  The log transformation produces a roughly Gaussian
 distribution, with a hint of fat tails for cheaper properties with prices near
 \$25,000.  The presence of a long lower tail might indicate that the data mix
 typical home sales with special, subsided properties or perhaps vacant lots
 that sell for an unusually low price.  (This display uses the base 10 log of
 the prices for easy interpretation; subsequent models use natural logs.)
 ...
 Here are four of the property listings.  The initial value is the listed price.
 ...
 \noindent
 Listings do not obey the grammatical rules of English.  Rather, listings are
 written in an idiosyncratic vernacular familiar only to those who are house
 hunting.  Some authors write in sentences, others not, and a variety of
 ...
 several words to a long paragraph.  The average description has about 73 words,
 but the distribution of the lengths is right skewed.  The boxplot in Figure
 \ref{fig:boxplot} shows the lengths.  The shortest description has 2 words,
 whereas the longest has 568 words.  This variation in the lengths of the
 descriptions suggests that modeling the prices from this text will require some
 form of weighting: we simply do not know so much about properties with short
 descriptions.


 \begin{figure}
 \caption{ \label{fig:boxplot} { \sl The distribution of the lengths of the
 property descriptions is right-skewed, with some listing running hundreds of
 ...
 A natural approach for a statistician is to construct intuitively appealing
 regressors from the descriptions.  For example when processing ads for real
 estate, one might conjecture that an agent has a lot more to say about an
 expensive property than one in need of repair, implying that the length of the
 listing for a property may be predictive of the price.  In a different vein,
 one can use regular expressions to extract specific numerical data from the
 listings.  For example, the first listing indicates that this property has two
 bedrooms and two bathrooms.  Constructing such regular expressions, however, is
 a labor-intensive process that must be done on a case-by-case basis.  The
 patterns used in these regular expressions become complex because they must
 allow common abbreviations, such as ``bth'', ``bath'' and ``bthrm'' for
 bathrooms.  Complexity aside, the greater problem with this approach is that
 most listings omit these characteristics.  The only numerical data common to
 every listing is the response, the advertised price.  Some listings include the
 number of rooms or occasionally the size of the property (number of square
 feet). For Chicago, our regular expressions found that 6\% of the listings
 indicate the number of square feet, 26\% indicate the number of bathrooms, and
 42\% give the number of bedrooms.  More complex regular expressions would
 likely find more matches, but the gains are likely to be few.  One also faces a
 Type I/Type II trade-off.  Simple regular expressions omit some matches, but
 more aggressive expressions match inadvertently.
 ...
 the number of words in descriptions.  To indicate missing data, we filled the
 missing values with the averages of the observed cases.  Missing values produce
 the columns of gray points located at the mean of the variable on the $x$ axis
 in Figure \ref{fig:parsed}.  The correlations of these extracted variables with
 the log of the prices range from ($r=0.40$) for length of the description and
 the number of bathrooms ($r = ??$) and log sq ft) $= 0.26$ down to. ???  These
 plots also show several anomalies.  For example, the scatterplot of the log of
 price on the number of words shows a cluster of 25 listings, all with exactly
 265 words.  All of these different properties were listed in a common format by
 a government agency.  The scatterplot of the log of prices on the log of the
 square footage also shows a clear outlier; this outlier is a consequence of
 aggressive parsing.  A typo in a description (``has 1sfam'') led to the regular
 expression defining a property with 1 square foot.
 ...
 correlation uses only the observed data; the adjacent value in gray uses all of
 the data. }
 \centerline{
 \vspace{0.1in}
 ...
 on these four extracted variables.  To accommodate missing values, we include
 in the fit missing indicators for the variables given by the regular
 expressions.  The regressors are not highly correlated, and the importance of
 these variables in the multiple regression generally echoes the marginal
 correlations shown in Figure \ref{fig:parsed}.  Among the seven explanatory
 variables, the length of the descriptions is most predictive, followed by the
 ...
 significant.  The missing indicators for bedrooms and bathrooms are not
 predictive.  The model obtains adjusted R-squared $\ol{R}^2 = 0.192$, with
 residual standard deviation $s_e =1.08$.  Because we compare regression models
 with varying numbers of explanatory variables, $\ol{R}^2$ is more useful than
 without adjusting for degrees of freedom.  Since we are not doing variable
 selection at this point, but rather fitting full ensembles of pre-selected
 variables, adjusted R-squared is an unbiased summary of each model's
 performance out of sample.  Later in Section \ref{sec:cv} we reports
 cross-validation results for these models.
...
\end{table}
 ...
 characterisitics, such constructive modeling would be more successful.  Our
 goal here, however, is to illustrate automatic methods that work in precisely
 this unformatted, unregulated context.  The collection of methods we use are
 collectively known as {\em vector space models} in computational linguistics
 \citep[e.g.][]{turney10}.  These were originally developed for document
 retrival, known as latent semantic indexing (\ras{cite}).  In that context,
 documents are clustered to enable a query to locate related documents or to
 partition documents into related clusters.  In contrast to featurizing text for
 regression modeling, these applications of vector space models are
 unsupervised.  Vector space models are also closely related to techniques that
 statisticians will find familiar: principal components analysis (PCA) and
 canonical correlation analysis (CCA).  The underlying computational engine is a
 singular value decomposition (SVD), aided by random projections.  Our aim is to
 show how easily one can convert text into familiar regressors for regression.
  As such, we leave to others the task of attempting to explain why such simple
 representations as the co-occurrence of words in documents might capture the
