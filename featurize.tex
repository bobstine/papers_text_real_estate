...
Topic models are a natural means to motivate vector space models, but topic models do not capture an important aspect of these data.  Namely, the authors of the descriptions write more about valuable properties than cheaper properties.  Descriptions of more expensive properties, those with numerous attractive attributes are longer.  Also, a description only mentions an attribute, such as a fireplace, when the attribute is present.  An ad would not mention that a property did not have a fireplace.  The following model similarly describes distributions over words in such a way that the presence and abundance of some words is indicative of higher prices.


\clearpage


Each real-estate property described by a listing has a collection of attributes that determines the value of the property.   Examples of attributes are a fireplace or hardwood floors.  We model these attributes as a randomly chosen subset from the collection of all possible attributes.  Let ${\cal A} = \{a_1,\ldots, a_K\}$ denote the collection of $K$ possible attributes,  and let ${\cal L}_i \subset {\cal A}$ denote the attributes of the property described by the $i$th listing.  We model the number of attributes present, $|{\cal L}_i|$, as a negative binomial distribution,
\begin{equation}
   |{\cal L}_i|  \sim 1 + \mbox{Poisson}(\xi), \quad \xi \sim \mbox{Gamma}(\al,\beta) \;,
\end{equation}
Every document has at least 1 attribute.  Gamma$(\al,\beta)$ denotes a gamma random variable with shape parameter $\al$ and scale $\beta$ (with mean $\al\,\beta$).  Let the $n \times K$ binary matrix $A$ hold indicators of which attributes appear in each document,
\begin{equation}
  \underset{n \times K}{A} = \left[ A_{ij} =
              \left\{ \begin{array}{cc} 1 & \mbox{ if } a_k \in {\cal L}_i \cr 0 & \mbox{otherwise.} 
                       \end{array} \right. \right]  \;.
\end{equation}
The $i$th row of $A$ gives the attributes in the $i$th document; for example, $\sum_j A_{ij} = |{\cal L}_i|$. 

Attributes determine both the price and the text of the listing through a stochastic mechanism.  For the price, we assume that each attribute has an effect $\mu_k  = \mu(a_k)$ on the expected price of the listing.  Arrange these in a vector $\bmu = (\mu_1,\ldots,\mu_K)'$.  The listed price is then determined by combining the contributions of its attributes.  Each attribute in ${\cal L}_i$ independently adds a random quantity $Q_{ik} \sim N(\mu_k, \sigma_q^2)$ to the response. [Might ought to have variance related to size?]   Each contribution is stochastic: the benefit of a fireplace is not a fixed constant.  The observed prices $Y$ for the listings are
\begin{equation*}
  Y = \exp(Q)
\end{equation*}
where $Q = (Q_1,\ldots,Q_n)'$ are random variables that sum the attribute effects for the separate listings,
\begin{equation}
   Q_i = \sum_{a_k \in {\cal L}_i}  Q_{ik}
                    \sim N \left(\sum_{a_k \in {\cal L}_i} \mu_k, \;
                                      |{\cal L}_i| \sigma_q^2 + \sigma_u^2\right)  \;.
\end{equation}
Notice that the effects of attributes on price are multiplicative (price is log normal).  The variance  $\sigma_u^2$ represents random noise.  We would not know the price even if we knew the observed effect $Q_k$ of every attribute.  The expected value of the vector $Y = (Y_1,\ldots,Y_n)'$ of log prices has the compact form
\begin{equation}
  \ev \log Y = A \bmu \;.
\end{equation}

Each attribute defines a multinomial distribution over the vocabulary of words. The words in a listing are then sampled according to this distribution.  The parameters of these multinomial distributions are themselves sampled from two Dirichlet distributions.  One Dirichlet distribution determines the probabilities for common words that are shared among attributes; the second places a distribution over words specific to each attribute.  For our example, we have $m_c$ ``common'' words that are shared over the attributes, and $m - m_c$ attribute-specific words.  Attributes use a wider collection of common words than attribute-specific words, so the distribution over attribute-specific words is more spiky.  (To simulate a sample of $n$ from a Dirichlet distribution with parameter $\al>0$, generate $x_1,\ldots,x_n \sim \mbox{Gamma}(\al)$ and return $x_i/S$ with $S = \sum_i x_i$.  Small values of $\al$ produce very skewed distributions and so concentrate the multinomial distribution on a limited set.)  Let $p_{k} = (p_{k1},\ldots,p_{km})$ with $0 \le p_{ki} \le 1$ and $\sum p_{ki} = 1$ denote the distribution over the vocabulary of $m$  words for attribute $k$, $k = 1,\ldots,K$. The elements of $p_k$ are
\begin{equation}
  2 \, p_{k,1:m_c} \sim \mbox{Dir}(0.2), \quad 2 \, p_{k,m_c+1:m} \sim \mbox{Dir}(0.005)
\end{equation}
...
The expected word frequencies for a document are then sums of selected rows of $P$, further scaled by a parameter $\lambda$.  $\la$ determines the average number of words per attribute.  Let $W$ denote the $n \times m$ matrix with the frequencies of word types (the document/word matrix).  Then
\begin{equation}
