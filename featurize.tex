...
The first $k$ columns of $Q_\ell U_c$ approximate  $U_{1:k}$. \citet{tropp10}
 ...
 The spectrum of the context-word matrix $W$ lacks the distinct gap that would suggest the rank needed to obtain an accurate low-rank approximation.  Figure \ref{fig:spectrum} graphs the singular values of $\widetilde{W}$ on a log scale. (Figure \ref{fig:spectrum} required computing the exact SVD of $\widetilde{W}$ to obtain the singular values.  Other scalings of $W$ produce similar spectra. See also the discussion of Figures \ref{fig:spectra} and \ref{fig:spectratwo} in Section \ref{sec:topic}.)  After the first few elements, the singular values fall off steadily, without a clear gap that would identify an appropriate rank.  The absence of a clear rank complicates the choice of how many components to use in a regression, so once again we use $AIC_c$.  In this case, we order the features by singular value.
 
 
 \begin{figure}
 \caption{ 
 	\label{fig:spectrum}
	{\sl The spectrum of the scaled document-term matrix $\widetilde{W}$ decays relatively slowly 
...
%--------------------------------------------------------------------------
\section{More Features: CCA and Bigrams}
\label{sec:cca}
%--------------------------------------------------------------------------

We can recast the frequencies in the document-word matrix $W$ as covariances between two types of indicator variables. This alternative perspective provides a basis for creating a wide range of features from text.  The key notion that distinguishes this perspective is to think of text as defining a large binary matrix.  Recall that we have $n$ real-estate listings that we will now generically call `documents'.   Document $i$ has $n_i$ word tokens that have been chosen from a vocabulary of $m$ word types.  


Indicator variables convert the text of these documents into a large, sparse matrix.  Begin by concatenating the word tokens of every document into a list, say
\begin{equation*}
...
This conversion produces more than numbers.  It also changes the way we interpret other computations, such as the word frequencies $W$ used in LSA.  To obtain these counts, define an $N \times n$ context matrix $L$  that identifies the documents,
\begin{equation}
  L =  \left(  
           \begin{array}{ccccc}
            1  & 0 & 0 & \cdots & 0 \cr
...
\end{equation}
where $\one{n}$ denotes a column vector of $n$ 1s. (This definition abuses the Kronecker product by allowing the right-hand vectors to have unequal length.)  The document-word matrix $W$ is then $N-1$ times the sample covariance matrix between the columns of $X$ and $L$,
\begin{equation*}
   W = (N-1) \cov(L,X) = L'X  \;.
\end{equation*}
...
In this first example, we consider how well estimation methods work when documents are generated by attributes with nearly disjoint specific words.  All attributes share the common Zipf distribution in this context, but have little overlap in the choice of specific words.  To obtain this behavior, we set the Dirichlet parameter in \eqn{ps} to $\al = 0.01$.   
