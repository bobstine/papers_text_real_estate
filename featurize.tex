...
The use of LSA to create features for regression models essentially reproduces principal components regression.  The only differences are the absence of centering and the variety of approaches for weighting counts prior to constructing the principal components. For example, one might choose to stabilize the variance of the counts.  Define ${W}^{*}$ to be the $n \times m$ matrix with elements ${W}^{*}_{ij} = W_{ij}/\sqrt{n_i}$ with $n_i = \sum_j W_{ij}$ counting the number of word tokens in the $i$th document.  Were tokens within listings random samples from
 ...
 of $W_k$ can be very slow.  Since we only require the portion of the range of $W$ captured by $U_k$, however, we can 
...
The conversion of words into LSA components produces a qualitative change in the structure of the $t$-statistics.  Figure \ref{fig:lsatstats} summarizes (in the same format as Figure \ref{fig:aictstats}) the $|t|$-statistics from a regression using the first 1,000 LSA component features. (As with words, the model includes a polynomial in document length.) Unlike the regression directly on frequencies, the $|t|$-statistics are roughly monotonically decreasing, with the most significant effects present in the initial components and gradually falling off. Sorting based on singular value is far more useful than sorting on word frequency.  The fitted model obtains $R^2 = 0.718$ with $\ol{R}^2 =  0.674$, compared to $\ol{R}^2 = 0.654$ for the previous regression on 1,089 words.  Though the $t$-statistics are more conveniently positioned by LSA than those in the regression directly on word frequencies, these models share similar explanatory power.  Alternatively, $AICc$ selects the model that uses 523 LSA components, obtaining $R^2 = 0.677$ and $\ol{R}^2  = 0.652$.  

% figure lsaaic.pdf has the plot of the R2 for this model.

\begin{figure}
\caption{  \label{fig:lsatstats}  
  {\sl Absolute $t$-statistics from the regression of log prices on the first 1,500 LSA components with column weighting.}  }
