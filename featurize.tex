...
% ----------------------------------------------------------------------
 ...
 measurements with physician comments.  Analogously, online product ratings shown
 at Amazon blend explicit characteristics with verbal commentary.  How can one easily incorporate the information in such text into a regression model that predicts patient outcomes or product ratings?  Our objective here is to illustrate several particularly straightforward techniques for featurizing text, turning text into numerical variables suitable for regression modeling.  
 
 
  As a specific example, we build a regression model to predict the advertised prices of real
 estate properties from their listings.  The listings we use are verbal rather than
 numerical data such as that obtained by filling out a spreadsheet-like questionnaire.  The listings may, or may not, contain quantitative characteristics.   The raw data are $n=$7,384 property listings for Chicago, IL extracted (with permission) from trulia.com on June 12, 2013.   (At the time, trulia.com showed  30,322 listings for Chicago, but most were foreclosures that we excluded from our analysis.) The response in our models is the log of the listed price.  The distribution of the listed prices is right skewed, so we transformed the response to a log scale as shown in the histogram of Figure \ref{fig:prices}.  The log transformation produces a roughly Gaussian distribution, with a hint of fat tails for cheaper properties with prices near \$25,000.  The presence of a long lower tail might indicate that the data
 mix typical home sales with special, subsided properties or perhaps vacant lots
 that sell for an unusually low price.  (This display uses the base 10 log of the prices for easy interpretation; subsequent models use natural logs.)
 ...
 \noindent
Listings do not obey the grammatical rules of  English.  Rather, listings are written in an idiosyncratic vernacular familiar only to those who are
 ...
 several words to a long paragraph.  The average description has about 73 words, but the distribution of the
 lengths is right skewed.  The boxplot in Figure \ref{fig:boxplot} shows
 the lengths.  The shortest description has 2 words, whereas the longest
  has 568 words.  This variation in the lengths of the descriptions
 suggests that modeling the prices from this text will require some form of weighting: we simply do not know so much about  properties with short
 descriptions.


 \begin{figure}
 \caption{ \label{fig:boxplot} { \sl The distribution of the lengths of the property descriptions is right-skewed, 
 with some listing running hundreds of words compared to a median length of 74 words. } }
 \centerline{
 \vspace{0.1in}
 ...
 A natural approach for a statistician is to construct intuitively appealing regressors from the descriptions.  For example when
 processing ads for real estate, one might conjecture that an agent has a lot
 more to say about an expensive property than one in need of repair, implying that the length of the listing for a property may be predictive of the price.  In a different vein, one can use regular
 expressions to extract specific numerical data from the listings.  For example, the first listing indicates that this property has two bedrooms and two bathrooms.  Constructing such regular expressions, however, is a labor-intensive process that must be done on a case-by-case basis.  The patterns used in these regular expressions become complex because they must  allow common abbreviations, such as ``bth'', ``bath'' and ``bthrm'' for
 bathrooms.  Complexity aside, the greater problem with this approach is that most listings omit these
 characteristics.  The only numerical data common to every listing is the response, the advertised price.  Some listings include the number of rooms or occasionally the size of the property (number of square feet). For Chicago, our regular expressions found that 6\% of the
 ...
 the number of words in descriptions.   To indicate missing data, we filled the missing values with the averages of the observed cases.  Missing values produce the columns of
 gray points located at the mean of the variable on the $x$ axis in Figure \ref{fig:parsed}.  The correlations of these extracted variables with the log of the prices range from ($r=0.40$) for length of the description  and the number of bathrooms
 ($r = ??$) and log sq ft) $= 0.26$ down to. ???    These plots also show several  anomalies.  For example, the   scatterplot of the log of price on the number of words shows a cluster of
 ...
 the regular expression defining a property with 1 square foot.
 ...
 correlation uses only the observed data; the adjacent value in gray uses all of the data. }
 ...
 model's performance out of sample.  Later in Section \ref{sec:cv} we reports cross-validation results for these models.
