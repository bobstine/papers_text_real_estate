...
\end{equation}
Every document has at least 1 attribute.  Gamma$(\al,\beta)$ denotes a gamma random variable with shape parameter $\al$ and scale $\beta$ (with mean $\al\,\beta$).  Let the $n \times K$ binary matrix $A$ hold indicators of which attributes appear in each document.  $A_{ij} = 1$ if attribute $j$ appears in document $i$ and is 0 otherwise so that, for example, $\sum_j A_{ij} = |{\cal L}_i|$. 

Attributes determine both the price and the text of the listing through a stochastic mechanism.  For the price, we assume that each attribute has an effect $\mu_k  = \mu(a_k)$ on the expected price of the listing.  The listed price for the $i$th listing is then determined by combining the contributions of its attributes as follows.  Each attribute that is present independently adds a stochastic quantity $Q_{ik} \sim N(\mu_k, \sigma_q^2)$ to the response for this listing. [Might ought to have variance related to size?]   Each contribution is stochastic: the benefit of a fireplace is not a fixed constant.  The observed price $Y_i$ for the $i$th listing is then
\begin{equation}
  Y_i = \exp(Q_i), \quad Q_i = \sum_{a_k \in {\cal L}_i}  Q_{ik}
                    \sim N \left(\sum_{a_k \in {\cal L}_i} \mu_k, \;
                                      |{\cal L}_i| \sigma_q^2 + \sigma_u^2\right)  \;.
\end{equation}
Notice that the effects on the price are multiplicative (price is log normal).  The variance  $\sigma_u^2$ represents random noise.  We would not know the price even if we knew the observed effect $Q_k$ of every attribute.

The choice of words used to describe each attribute is sampled from a multinomial distribution.  The parameters of these multinomial distributions are themselves sampled from two Dirichlet distributions.  One Dirichlet distribution determines the probabilities for common words; the second places a distribution over words specific to each attribute.  For our application, we have $m_c$ ``common'' words that are shared over the attributes, and $m - m_c$ attribute-specific words.  Attributes use a wider collection of common words than attribute specific words, so the distribution over attribute-specific words is more spiky.  (To simulate a sample of $n$ from a Dirichlet distribution with parameter $\al>0$, generate $x_1,\ldots,x_n \sim \mbox{Gamma}(\al)$ and return $x_i/S$ with $S = \sum_i x_i$.  Small values of $\al$ produce very skewed distributions and so concentrate the multinomial distribution on a limited set.)  Let $p_{k} = (p_{k1},\ldots,p_{km})$ with $0 \le p_{ki} \le 1$ and $\sum p_{ki} = 1$ denote the distribution over the vocabulary of $m$  words for attribute $k$, $k = 1,\ldots,K$. The elements of $p_k$ are
\begin{equation}
